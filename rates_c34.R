...
# Cluster 34
####################################################################################################################
spread3_all <- read_tsv("C:/Users/gev25289/Desktop/december clusters/rep0/c34_spread.txt") 
...
logfile <- read_delim("C:/Users/gev25289/Desktop/december clusters/rep0/rep0.c34.combined.location.rates.log")
...
ggsave("C:/Users/gev25289/Desktop/december clusters/rep0/c34_color_rates_grid.pdf", combined_plot_colors, width = 23, height = 7, units = "cm")
all_plot_colors
ggsave("C:/Users/gev25289/Desktop/december clusters/rep0/c34_color_rates.pdf", all_plot_colors, width = 22, height = 7, units = "cm")
...
combined_plot_thick
#ggsave("C:/Users/gev25289/Desktop/december clusters/rep0/c34_thickness_rates_grid.pdf", combined_plot_thick, width = 23, height = 7, units = "cm")

all_plot_thick
ggsave("C:/Users/gev25289/Desktop/december clusters/rep0/c34_thickness_rates.pdf", all_plot_thick, width = 23, height = 7, units = "cm")

# To combine with other subsamples later
# Create a title grob
title_grob <- ggdraw() + 
  draw_label("Cluster 34", size = 10, fontface = "bold", hjust = 0.5, vjust = 3)

# Combine the title and the final plot
rep0_c34_combined_plot_thick <- plot_grid(
...
rep0_c34_combined_plot_thick

rep0_c34_all_plot_thick <- plot_grid(
  title_grob,          # Add the title grob
...
bf_combined_plot <- plot_grid(#substantial_plot, 
  ...
  ncol = 1)
# ggsave("C:/Users/gev25289/Desktop/december clusters/rep0/c34_bf_grid.pdf", bf_combined_plot, width = 3, height = 8, units = "cm")

#To combine with other subsamples later
# Create a title grob
title_grob <- ggdraw() + 
  draw_label("Cluster 34", size = 8, fontface = "bold", hjust = 0.5)

# Combine the title and the plot using plot_grid
rep0_c34_bf_combined_plot <- plot_grid(
  title_grob,           # Add the title grob
...
rm(list = setdiff(ls(), c("rep0_c9_bf_combined_plot", "rep0_c9_combined_plot_thick", "rep0_c9_all_plot_thick", "rep0_spread9", "spread3_50_c9",
                          "rep0_c34_bf_combined_plot", "rep0_c34_combined_plot_thick", "rep0_c34_all_plot_thick", "rep0_spread34", "spread3_50_c34")))

# Combine
combined_all_plot_thick <- plot_grid(rep0_c9_all_plot_thick,
                                     rep0_c34_all_plot_thick, ncol = 2)
ggsave("C:/Users/gev25289/Desktop/december clusters/figures/rep0_c9_c34_combined_all_plot_thick.pdf", combined_all_plot_thick, width = 15, height = 10, units = "cm")

combined_bf_plot <- plot_grid(rep0_c9_bf_combined_plot,
                              rep0_c34_bf_combined_plot,
                              ncol =2)
ggsave("C:/Users/gev25289/Desktop/december clusters/figures/rep0_c9_c34_combined_bf_plot.pdf", combined_bf_plot, width = 6, height = 9, units = "cm")

#Decisive common
####################################################################################################
spread_c9 <- rep0_spread9 %>% mutate(c = "c9") #9
spread_c34 <- rep0_spread34 %>% mutate(c = "c34") #16
spread <- bind_rows(spread_c9, spread_c34) %>% mutate(FromTo = paste(FROM, TO, sep="."))
commonrates <- spread %>% group_by(FromTo, FROM, TO) %>% summarize(n=n()) %>% filter(n==2) #0
# Cluster 9 - 4/9 decisive rates with a source of South_Central
# Cluster 34 - 13/16 decisive rates with a source of DeKalb

#All common
spread_c9 <- spread3_50_c9 %>% mutate(c = "c9") #25
spread_c34 <- spread3_50_c34 %>% mutate(c = "c34") #30
spread <- bind_rows(spread_c9, spread_c34) %>% mutate(FromTo = paste(FROM, TO, sep=))
commonrates <- spread %>% group_by(FromTo, FROM, TO) %>% summarize(n=n()) %>% filter(n==2) #4
#If we consider all supported rates instead of just decisive, there are 4 in common: Clayton_DeKalb, Clayton_District4, ClaytonNorthwest, and SouthCentralDeKalb
