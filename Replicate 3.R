...
# Metadata
metadata <- read_tsv(gzfile("/scratch/gev25289/workdir/georgia/delta/ncov/rep3/results/georgia3/metadata_adjusted.tsv.xz"))
...
metadata %>% group_by(division) %>% summarize(n=n()) #Georgia, n= 4978; OOS, n = 4845

# Import annotated trees - have to use read.beast to retain location and date annotations
timetree_noci <- read.beast("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep3/2025-01-02_treetime/timetree.nexus") #has no CI info
timetree_noci #Phylogenetic tree with 9824 tips and 3151 internal nodes (date annotations, no location)
...
#write.tree(timetree_noci_phylo, "/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep3/2025-01-02_treetime/timetree.nwk")

# Skip first row: "#Lower and upper bound delineate the 90% max posterior region"
dates <- read_tsv("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep3/2025-01-02_treetime/dates.tsv", skip = 1) %>%
  rename(date_chr = date, label="#node") #Confidence Intervals

# Read mugration tree
new.tree <- read.beast("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep3/2025-01-02_mugration/annotated_tree.nexus")
new.tree #9824 tips and 3151 internal nodes
...
# 2057 supported nodes (65%) - aBayes criteria makes no difference, same results without it
Nnode(new.tree) #3151 nodes
...
imports <- phylodata_j %>% filter(event == "import") #there are 284 import events, including singletons
...
getMRCA(new.tree@phylo, georgia) #mrca of all georgia sequences in tree = 9826
...
summary(nodelist3$No.Total) #the cluster size ranges from 3 to 1137 seqs, median 10.5 seqs
summary(nodelist_supported$No.Total) #the cluster size ranges from 3 to 402 seqs, median 7.5 seqs

summary(nodelist3$meanPer) #ga seqs range from 44-100% of the total cluster size, median 85%
summary(nodelist_supported$meanPer) #ga seqs range from 44-100% of the total cluster size, median 92%
...
saveRDS(clusters, "/scratch/gev25289/workdir/georgia/delta/rds/rep3_cluster_tips.rds")
...
p284 <- p + geom_hilight(data=imports, mapping=aes(node=node),
                         alpha=0.2, fill="darkred", color="black",
                         size=0.4, extend = 0.1) 
p54 <- p + geom_hilight(data=nodelist3, mapping=aes(node=Node),
...
plot_grid(p284, p54, p34, ncol=3, labels = c("A", "B", "C"))
...
fasta <- phylotools::read.fasta("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep3/masked_filtered_rep3.fasta") 
...
phylotools::dat2fasta(combined_fasta, outfile = "/scratch/gev25289/workdir/georgia/delta/clusters/rep3/all_clusters_fasta.fasta") #don't need to keep updating this

# Fasta file of each cluster
output_dir <- "/scratch/gev25289/workdir/georgia/delta/clusters/rep3/"
