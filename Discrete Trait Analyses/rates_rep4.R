...
####################################################################################################################
spread3_all <- read_tsv("C:/Users/gev25289/Desktop/december clusters/rep4/spread.txt") 
...
logfile <- read_delim("C:/Users/gev25289/Desktop/december clusters/rep4/rep4.combined.location.rates.log")
...
ggsave("C:/Users/gev25289/Desktop/december clusters/rep4/color_rates_grid.pdf", combined_plot_colors, width = 23, height = 7, units = "cm")
all_plot_colors
ggsave("C:/Users/gev25289/Desktop/december clusters/rep4/color_rates.pdf", all_plot_colors, width = 22, height = 7, units = "cm")
...
combined_plot_thick
ggsave("C:/Users/gev25289/Desktop/december clusters/rep4/thickness_rates_grid.pdf", combined_plot_thick, width = 23, height = 7, units = "cm")

all_plot_thick
ggsave("C:/Users/gev25289/Desktop/december clusters/rep4/thickness_rates.pdf", all_plot_thick, width = 23, height = 7, units = "cm")

# To combine with other subsamples later
# Create a title grob
title_grob <- ggdraw() + 
  draw_label("Replicate 4", size = 10, fontface = "bold", hjust = 0.5, vjust = 3)

# Combine the title and the final plot
rep4_combined_plot_thick <- plot_grid(
...
rep4_combined_plot_thick

rep4_all_plot_thick <- plot_grid(
  title_grob,          # Add the title grob
...
bf_combined_plot <- plot_grid(#substantial_plot, 
  ...
  ncol = 1)
ggsave("C:/Users/gev25289/Desktop/december clusters/rep4/bf_grid.pdf", bf_combined_plot, width = 3, height = 8, units = "cm")

#To combine with other subsamples later
# Create a title grob
title_grob <- ggdraw() + 
  draw_label("Replicate 4", size = 8, fontface = "bold", hjust = 0.5)

# Combine the title and the plot using plot_grid
rep4_bf_combined_plot <- plot_grid(
  title_grob,           # Add the title grob
...
rm(list = setdiff(ls(), c("rep0_bf_combined_plot", "rep0_combined_plot_thick", "rep0_all_plot_thick", "spread_rep0",
                          ...
                          "rep4_bf_combined_plot", "rep4_combined_plot_thick", "rep4_all_plot_thick", "spread_rep4")))

# Combine all subsamples
combined_plot_thick <- plot_grid(rep0_combined_plot_thick, 
                                 rep1_combined_plot_thick, 
                                 rep2_combined_plot_thick, 
                                 rep3_combined_plot_thick, 
                                 rep4_combined_plot_thick, ncol = 1)
ggsave("C:/Users/gev25289/Desktop/december clusters/figures/combined_plot_thick.pdf", combined_plot_thick, width = 40, height = 50, units = "cm")

library(patchwork)

rep0_all_plot_thick_title <- ggdraw() + 
  draw_plot(rep0_all_plot_thick) + 
  draw_label("Replicate 0", x = 0.5, y = 1, vjust = 1.5, fontface = 'bold', size = 8)

rep1_all_plot_thick_title <- ggdraw() + 
  draw_plot(rep1_all_plot_thick) + 
  draw_label("Replicate 1", x = 0.5, y = 1, vjust = 1.5, fontface = 'bold', size = 8)

rep2_all_plot_thick_title <- ggdraw() + 
  draw_plot(rep2_all_plot_thick) + 
  draw_label("Replicate 2", x = 0.5, y = 1, vjust = 1.5, fontface = 'bold', size = 8)

rep3_all_plot_thick_title <- ggdraw() + 
  draw_plot(rep3_all_plot_thick) + 
  draw_label("Replicate 3", x = 0.5, y = 1, vjust = 1.5, fontface = 'bold', size = 8)

rep4_all_plot_thick_title <- ggdraw() + 
  draw_plot(rep4_all_plot_thick) + 
  draw_label("Replicate 4", x = 0.5, y = 1, vjust = 1.5, fontface = 'bold', size = 8)

# Combine the plots with titles
combined_all_plot_thick <- plot_grid(
  rep0_all_plot_thick_title,
  rep1_all_plot_thick_title,
  rep2_all_plot_thick_title,
  rep3_all_plot_thick_title,
  rep4_all_plot_thick_title,
  ncol = 1,
  align = 'v'
)
combined_all_plot_thick
ggsave("C:/Users/gev25289/Desktop/december clusters/figures/combined_all_plot_thick.pdf", combined_all_plot_thick, width = 15, height = 20, units = "cm")

combined_bf_plot <- plot_grid(rep0_bf_combined_plot,
                              rep1_bf_combined_plot,
                              rep2_bf_combined_plot,
                              rep3_bf_combined_plot,
                              rep4_bf_combined_plot,
                              ncol =5)
ggsave("C:/Users/gev25289/Desktop/december clusters/figures/combined_bf_plot.pdf", combined_bf_plot, width = 20, height = 15, units = "cm")

spread_rep0 <- spread_rep0 %>% mutate(rep = 0)
spread_rep1 <- spread_rep1 %>% mutate(rep = 1)
spread_rep2 <- spread_rep2 %>% mutate(rep = 2)
spread_rep3 <- spread_rep3 %>% mutate(rep = 3)
spread_rep4 <- spread_rep4 %>% mutate(rep = 4)

spread <- bind_rows(spread_rep0, spread_rep1, spread_rep2, spread_rep3, spread_rep4)
commonrates <- spread %>% group_by(FromTo, FROM, TO) %>% summarize(n=n()) %>% filter(n==5) 
...
#Rep 0 has 34 decisive rates, Rep 1 has 17, Rep 2 has 22, Rep 3 has 24, and Rep 4 has 20. 
#There are 5 that are common across the board
table(commonrates$FromTo) #South Central is the source to Clayton, Coastal, DeKalb, East Central, 
                          #Fulton, GNR, North Central, Northeast. Plus, North Georgia -> South Central

names <- c("Clayton", "North Georgia", "Coastal", "DeKalb", "East Central", "Fulton", "GNR", "North Central", "Northeast")

commonrates_plot <- combine %>%
  ggplot() +  
  geom_sf(aes(fill = public_health_district %in% names), color = "black", size = 2) +
  geom_curve(data = commonrates, aes(x = Longitude.x, y = Latitude.x, xend = Longitude.y, yend = Latitude.y), 
             size = 0.3,
             arrow = arrow(length = unit(0.03, "npc"), type="closed", angle=15)) +
  xlab("") + ylab("") + labs(size = "Location Exchange Rate") +
  theme(text = element_text(family = "sans")) +
  ...
  expand_limits(x = c(min(spread$Longitude.x, spread$Longitude.y) - 1, max(spread$Longitude.x, spread$Longitude.y) + 1), 
                y = c(min(spread$Latitude.x, spread$Latitude.y) - 1, max(spread$Latitude.x, spread$Latitude.y) + 1)) +
  scale_fill_manual(values = c("TRUE" = "palegreen", "FALSE" = "white")) +
  guides(fill="none")
ggsave("C:/Users/gev25289/Desktop/december clusters/figures/commonrates_plot.pdf", commonrates_plot, width = 6, height = 6, units = "cm")