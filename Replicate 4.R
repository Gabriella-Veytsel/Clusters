...
ClusterLevels30 <- c("Cluster 1", "Cluster 2", "Cluster 3", "Cluster 4", "Cluster 5", "Cluster 6", "Cluster 7", "Cluster 8", "Cluster 9", 
...
# Metadata
metadata <- read_tsv(gzfile("/scratch/gev25289/workdir/georgia/delta/ncov/rep4/results/georgia4/metadata_adjusted.tsv.xz"))
...
timetree_noci <- read.beast("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep4/2025-01-02_treetime/timetree.nexus") #has no CI info
timetree_noci #Phylogenetic tree with 9816 tips and 3152 internal nodes (date annotations, no location)
...
#write.tree(timetree_noci_phylo, "/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep4/2025-01-02_treetime/timetree.nwk")

# Skip first row: "#Lower and upper bound delineate the 90% max posterior region"
dates <- read_tsv("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep4/2025-01-02_treetime/dates.tsv", skip = 1) %>%
  rename(date_chr = date, label="#node") #Confidence Intervals

# Read mugration tree
new.tree <- read.beast("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep4/2025-01-02_mugration/annotated_tree.nexus")
new.tree #9816 tips and 3152 internal nodes
...
# 2030 supported nodes (64%) - aBayes criteria makes no difference, same results without it
Nnode(new.tree) #3152 nodes
...
imports <- phylodata_j %>% filter(event == "import") #there are 293 import events, including singletons
...
getMRCA(new.tree@phylo, georgia) #mrca of all georgia sequences in tree = 9818
...
nodelist3 <- nodelist %>% 
  filter(No.Georgia >= 3) #49

# Clusters should not overlap in sequences
nodelist_supported <- nodelist3 %>% filter(Node %in% supportedNodes) #30

# Leke's monophyletic function
vec_supported <- nodelist_supported %>% pull(Node) %>% unique() #Vector of nodes, 30

newNodes_supported <- selectMonophyleticGroup(new.tree@phylo, vec_supported) #30
...
summary(nodelist3$No.Total) #the cluster size ranges from 3 to 1135 seqs, median 17 seqs
summary(nodelist_supported$No.Total) #the cluster size ranges from 3 to 383 seqs, median 7 seqs

summary(nodelist3$meanPer) #ga seqs range from 55-100% of the total cluster size, median 83%
summary(nodelist_supported$meanPer) #ga seqs range from 55-100% of the total cluster size, median 86%
...
clusters <- lapply(1:30, clusterTips)
saveRDS(clusters, "/scratch/gev25289/workdir/georgia/delta/rds/rep4_cluster_tips.rds")
...
# clusters[[i]] %>% group_by(location) %>% summarize(n=n()) %>% nrow() #manual check
...
p293 <- p + geom_hilight(data=imports, mapping=aes(node=node),
                         alpha=0.2, fill="darkred", color="black",
                         size=0.4, extend = 0.1) 
p49 <- p + geom_hilight(data=nodelist3, mapping=aes(node=Node),
                        alpha=0.2, fill="yellow", color="black",
                        size=0.4, extend = 0.1) 
p30 <- p + geom_hilight(data=nodelist_supported, mapping=aes(node=Node),
                        alpha=0.2, fill="palegreen3", color="black",
                        size=0.4, extend = 0.1) 

plot_grid(p293, p49, p30, ncol=3, labels = c("A", "B", "C"))
...
bubbleplot <- ggplot(aggregated_clusters, aes(
  x = as.Date(date_chr), 
  y = factor(Cluster, levels = ClusterLevels30), size = n)) +
...
#Figure: TMRCA plot
################################################################################################################
tmrca_plot <- ggplot(nodelist_supported, aes(date_chr, y = factor(ClusterLevels30, levels = ClusterLevels30))) + 
...
fasta <- phylotools::read.fasta("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep4/masked_filtered_rep4.fasta") 
...
phylotools::dat2fasta(combined_fasta, outfile = "/scratch/gev25289/workdir/georgia/delta/clusters/rep4/all_clusters_fasta.fasta") #don't need to keep updating this

# Fasta file of each cluster
output_dir <- "/scratch/gev25289/workdir/georgia/delta/clusters/rep4/"
