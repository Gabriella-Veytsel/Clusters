...
# Metadata
metadata <- read_tsv(gzfile("/scratch/gev25289/workdir/georgia/delta/ncov/rep2/results/georgia2/metadata_adjusted.tsv.xz"))
metadata <- metadata %>% mutate(strain = gsub("/", "_", strain))
metadata %>% group_by(region) %>% summarize(n=n()) #6 continents
metadata %>% group_by(country) %>% summarize(n=n()) #124 countries
metadata %>% group_by(division) %>% summarize(n=n()) #Georgia, n= 4973; OOS, n = 4842

# Import annotated trees - have to use read.beast to retain location and date annotations
timetree_noci <- read.beast("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep2/2025-01-02_treetime/timetree.nexus") #has no CI info
timetree_noci #Phylogenetic tree with 9815 tips and 3095 internal nodes (date annotations, no location)
...
#write.tree(timetree_noci_phylo, "/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep2/2025-01-02_treetime/timetree.nwk")

# Skip first row: "#Lower and upper bound delineate the 90% max posterior region"
dates <- read_tsv("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep2/2025-01-02_treetime/dates.tsv", skip = 1) %>%
  rename(date_chr = date, label="#node") #Confidence Intervals

# Read mugration tree
new.tree <- read.beast("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep2/2025-01-02_mugration/annotated_tree.nexus")
new.tree #9815 tips and 3095 internal nodes
...
# 1976 supported nodes (64%) - aBayes criteria makes no difference, same results without it
Nnode(new.tree) #3095 nodes
...
imports <- phylodata_j %>% filter(event == "import") #there are 318 import events, including singletons
...
getMRCA(new.tree@phylo, georgia) #mrca of all georgia sequences in tree = 9816
...
summary(nodelist3$No.Total) #the cluster size ranges from 3 to 1138 seqs, median 10 seqs
summary(nodelist_supported$No.Total) #the cluster size ranges from 3 to 394 seqs, median 10 seqs

summary(nodelist3$meanPer) #ga seqs range from 53.8-100% of the total cluster size, median 90%
summary(nodelist_supported$meanPer) #ga seqs range from 60-100% of the total cluster size, median 94%
...
saveRDS(clusters, "/scratch/gev25289/workdir/georgia/delta/rds/rep2_cluster_tips.rds")
...
# clusters[[23]] %>% group_by(location) %>% summarize(n=n()) %>% nrow() #manual test
...
p318 <- p + geom_hilight(data=imports, mapping=aes(node=node),
                         alpha=0.2, fill="darkred", color="black",
                         size=0.4, extend = 0.1) 
p55 <- p + geom_hilight(data=nodelist3, mapping=aes(node=Node),
...
plot_grid(p318, p55, p34, ncol=3, labels = c("A", "B", "C"))
...
fasta <- phylotools::read.fasta("/scratch/gev25289/workdir/georgia/delta/treetime/treetime/rep2/masked_filtered_rep2.fasta") 
...
phylotools::dat2fasta(combined_fasta, outfile = "/scratch/gev25289/workdir/georgia/delta/clusters/rep2/all_clusters_fasta.fasta") #don't need to keep updating this

# Fasta file of each cluster
output_dir <- "/scratch/gev25289/workdir/georgia/delta/clusters/rep2/"
